...
public class NamedJdbcTemplate {
	...
	protected org.springframework.jdbc.core.namedparam.NamedParameterJdbcTemplate njdbcTemplate;
	...
	/**
	 * 根据sql语句，返回对象集合
	 * @param sql(参数用冒号加参数名，例如select * from tb where id=:id)
	 * @param clazz
	 * @param parameters(key为参数名，value为参数值)
	 * @return bean对象集合
	 */
	public List find(final String sql,Class clazz,Map parameters){
		try{
			...
			if(parameters!=null){
				//return njdbcTemplate.query(sql, resultBeanMapper(clazz),parameters);
                return njdbcTemplate.queryForList(sql,parameters,clazz);
			}else{
				return njdbcTemplate.queryForList(sql, parameters);
			}
		}catch (Exception e) {
			return null;
		}
	...
	public Object findForObject(final String sql,Class clazz,Map parameters){
		try{
			...
			Assert.notNull(clazz, "集合中对象类型不能为空!");
			if(parameters!=null){
				//return njdbcTemplate.queryForObject(sql, resultBeanMapper(clazz), parameters);
                return njdbcTemplate.queryForObject(sql,parameters,new BeanPropertyRowMapper(clazz));
			}else{
				//return njdbcTemplate.queryForLong(sql, resultBeanMapper(clazz));
                //return njdbcTemplate.queryForObject(sql,null,new BeanPropertyRowMapper(clazz));
                //njdbcTemplate.q
                return null;
			}
		}catch (Exception e) {
	...
	/**
	 * 根据sql语句，返回数值型返回结果
	 * @param sql(参数用冒号加参数名，例如select count(*) from tb where id=:id)
	 * @param parameters(key为参数名，value为参数值)
	 * @return bean对象
	 */
	public long findForLong(final String sql,Map parameters){
		try{
			Assert.hasText(sql, "sql语句不正确!");
			if(parameters!=null){
				//return njdbcTemplate.queryForLong(sql, parameters);
                return njdbcTemplate.queryForObject(sql,parameters,Long.class);
			}else{
				//return njdbcTemplate.queryForLong(sql);
                return 0;
			}
		}catch (Exception e) {
			return 0;
	...
	public Map findForMap(final String sql,Map parameters){
		try{
			Assert.hasText(sql,"sql语句不正确!");
			if(parameters!=null){
				//return njdbcTemplate.queryForMap(sql, parameters);
				return njdbcTemplate.queryForMap(sql, parameters);
			}else{
				//return njdbcTemplate.queryForMap(sql, paramMap)
				return njdbcTemplate.queryForMap(sql);
			}
		}catch (Exception e) {
			return null;
		}
	...
	public int executeForObject(final String sql,Object bean){
		Assert.hasText(sql, "sql语句不正确!");
		if(bean!=null){
			return njdbcTemplate.update(sql, paramBeanMapper(bean));
		}else{
            //return njdbcTemplate.update(sql,null);
			//return njdbcTemplate.update(sql);
            return 0;
		}
	}
	...
	public int executeForMap(final String sql,Map parameters){
		Assert.hasText(sql, "sql语句不正确!");
		if(parameters!=null){
			return njdbcTemplate.update(sql, parameters);
		}else{
            return 0;
		}
	}
	...
	public int[] batchUpdate(final String sql,List<Object[]> batch ){
        int[] updateCounts = njdbcTemplate.batchUpdate(sql,batch);
        return updateCounts;
	}*/
